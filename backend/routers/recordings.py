...
async def get_thumbnail(
    ...
    Thumbnail is generated by the recorder service when a recording
    completes (see services/recorder.py's _generate_thumbnail — same
    directory, same stem, .jpg extension). If generation failed or the
    recording predates this feature, this endpoint returns 404.

    Auth accepts a `token` query param (via get_current_user_flexible)
    since an <img> element can't send an Authorization header.
    """
    recording = await _get_recording_or_404(session, recording_id)
    thumb = _thumbnail_path(session, recording.filename)
